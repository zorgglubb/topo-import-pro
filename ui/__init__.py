...
from ..covadis_bretagne import (COVADIS_CATEGORIES, get_codes_by_category, decode_code,
    enrich_points as covadis_enrich, build_qgis_layers as covadis_layers,
    export_cod_2d, export_bpt, COVADIS_BRETAGNE_GENERAL)
from ..compat import make_field, log_info, log_warning
try:
    from ..covadis import get_codification, CODIFICATIONS, BretagneProcessor
    HAS_COVADIS = True
...
class TopoImportDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        self.tabs.addTab(self._wrap_scroll(self._tab_freestation()),  "📍 Station libre")
        ...
        self.tabs.addTab(self._wrap_scroll(self._tab_covadis()),      "🏺 COVADIS Bretagne")
        self.tabs.addTab(self._wrap_scroll(self._tab_results()),      "📋 Résultats")

        # Index de l'onglet Résultats, calculé dynamiquement (ne plus jamais
        # coder "6" en dur : le nombre d'onglets a déjà changé plusieurs fois).
        self._results_tab_index = self.tabs.count() - 1
    ...
    def _on_import_done(self, points):
        ...
        self._set_status(f"✅ {len(points)} point(s) importé(s) — {n_coord} avec coordonnées, {n_raw} mesures brutes")
        self._log(f"Import terminé : {len(points)} points ({n_coord} XYZ, {n_raw} bruts)")
    ...
    # ═════════════════════════════════════════════════════════════════
    # ONGLET COVADIS BRETAGNE
    # ═════════════════════════════════════════════════════════════════
    def _tab_covadis(self):
        w = QWidget()
        layout = QVBoxLayout(w)

        # ── Notice ────────────────────────────────────────────────────
        notice = QLabel(
            "<b>🏺 Codification COVADIS — Bretagne (ArchéoCOD Inrap 2017)</b><br>"
            ...
            "et exporte les fichiers .cod / .bpt natifs COVADIS."
        )
        notice.setWordWrap(True)
        notice.setStyleSheet(
        ...
        # ── Sous-onglets ──────────────────────────────────────────────
        sub_tabs = QTabWidget()
        sub_tabs.addTab(self._covadis_tab_decode(),   "🔍 Décodage & Couches")
        sub_tabs.addTab(self._covadis_tab_browse(),   "📖 Référentiel codes")
        sub_tabs.addTab(self._covadis_tab_export(),   "💾 Export .cod / .bpt")
        layout.addWidget(sub_tabs)
        return w

    # ── Sous-onglet 1 : Décodage & Couches QGIS ──────────────────────
    def _covadis_tab_decode(self):
        w = QWidget(); layout = QVBoxLayout(w)
        ...
        self.chk_cov_by_cat = QCheckBox("Une couche par catégorie COVADIS")
        self.chk_cov_by_cat.setChecked(True)
        self.chk_cov_style  = QCheckBox("Appliquer la couleur COVADIS (ACI)")
        self.chk_cov_style.setChecked(True)
        self.chk_cov_nocode = QCheckBox("Inclure les points sans code COVADIS")
        ...
        self.tbl_cov_preview.setHorizontalHeaderLabels(
            ["ID Point", "Code brut", "Libellé COVADIS", "Catégorie", "Calque"]
        )
        ...
        btn_prev.clicked.connect(self._covadis_preview)
        btn_prev.setStyleSheet("background:#F57F17; color:white; padding:5px;")
        btn_go = QPushButton("🏺 Créer couches COVADIS dans QGIS")
        btn_go.clicked.connect(self._covadis_create_layers)
    ...
    def _covadis_tab_browse(self):
        ...
        for cat in COVADIS_CATEGORIES:
            self.combo_cov_cat.addItem(cat)
        self.combo_cov_cat.currentTextChanged.connect(self._covadis_filter_ref)
        ...
        self.edit_cov_search.textChanged.connect(self._covadis_filter_ref)
        ...
        btn_load_ref.clicked.connect(self._covadis_load_ref)
        layout.addWidget(btn_load_ref)
        # Charger immédiatement
        self._covadis_load_ref()
        return w

    # ── Sous-onglet 3 : Export .cod / .bpt ───────────────────────────
    def _covadis_tab_export(self):
        w = QWidget(); layout = QVBoxLayout(w)

        lbl = QLabel(
            "Génère les fichiers natifs COVADIS compatibles avec :\n"
            "• COVADIS AutoCAD (fichier .cod chargeable directement)\n"
            "• Bibliothèques de points .bpt (un par type de levé)\n"
            "• Utilisables dans Covadis 14/15/16 et AutoCAD Map"
        )
        ...
        btn_exp = QPushButton("💾 Exporter les fichiers COVADIS")
        btn_exp.setStyleSheet("font-weight:bold; background:#1B5E20; color:white; padding:6px;")
        btn_exp.clicked.connect(self._covadis_export_files)
    ...
    def _covadis_preview(self):
        ...
        for p in enriched:
            ...
            for col, val in enumerate([
                ...
                getattr(p, 'covadis_categorie', 'Non reconnu'),
                getattr(p, 'covadis_calque', '—'),
            ]):
                item = QTableWidgetItem(val)
                item.setFlags(item.flags() & ~Qt.ItemIsEditable)
                # Colorier les lignes selon la couleur COVADIS
                if col == 3 and cat:
                    from qgis.PyQt.QtGui import QColor, QBrush
                    color = QColor(getattr(p, 'covadis_couleur', '#808080'))
                    color.setAlpha(60)
                    item.setBackground(QBrush(color))
                self.tbl_cov_preview.setItem(row, col, item)

        self._set_status(
            f"COVADIS : {n_reconnus}/{len(pts)} point(s) décodé(s) sur "
            f"{len(COVADIS_CATEGORIES)} catégories disponibles"
        )

    def _covadis_create_layers(self):
        """Crée les couches QGIS COVADIS."""
        pts = self._get_cov_points()
        if not pts:
        ...
        try:
            epsg = self.crs_selector.current_epsg()
            layers = covadis_layers(pts, crs_code=epsg)
            if not layers:
                QMessageBox.warning(self, "Résultat vide",
                    "Aucun code COVADIS reconnu dans les points.\n"
                    "Vérifiez que le champ 'Code' contient des codes COVADIS (ex: 10, 12#, 129...)")
                return
            for lyr in layers:
                QgsProject.instance().addMapLayer(lyr)
            self._log(f"COVADIS Bretagne : {len(layers)} couche(s) créée(s) dans QGIS")
            self._set_status(f"✅ {len(layers)} couche(s) COVADIS ajoutée(s) — CRS : {epsg}")
        except Exception as e:
            QMessageBox.critical(self, "Erreur COVADIS", str(e))

    def _covadis_load_ref(self):
        """Charge le référentiel complet des codes."""
        from ..covadis_bretagne import get_all_codes
        codes = get_all_codes()
    ...
    def _covadis_export_files(self):
        """Exporte les fichiers .cod et .bpt COVADIS."""
        ...
        try:
            ...
            for chk, nom, cp, cm, ca, cc in bpt_configs:
                if chk.isChecked():
                    ...
                    fname = covadis_names.get(nom, fname)
                    path = f"{folder}/{fname}"
                    with open(path, 'w', encoding='latin-1', errors='replace') as f:
            ...
            msg = f"✅ {len(exported)} fichier(s) COVADIS exporté(s) :\n" + "\n".join(f"  • {f}" for f in exported)
            QMessageBox.information(self, "Export COVADIS réussi", msg)
            self._set_status(f"✅ Export COVADIS : {len(exported)} fichiers → {folder}")
            self._log(f"Export COVADIS Bretagne : {', '.join(exported)}")
        except Exception as e:
            QMessageBox.critical(self, "Erreur export COVADIS", str(e))
    ...
    def _tab_covadis(self):
        ...
        # ── Notice ───────────────────────────────────────────────────
        notice = QLabel(
            "<b>🗺 Géocodification COVADIS — Bretagne (INRAP BZH 2017)</b><br>"
            "Décode les chaînes de codes COVADIS sur vos points importés, "
        ...
        if not HAS_COVADIS:
            lbl_err = QLabel(
                f"⚠ Module COVADIS non disponible : {_cov_err_msg if '_cov_err_msg' in dir() else 'fichiers .cod introuvables'}\n"
                "Vérifiez que les fichiers ArcheoCOD_2017-2D.cod et ArcheoCOD_2017-3D.cod "
                "sont présents dans le dossier covadis/ du plugin."
            )
        ...
        from ..covadis import CODIFICATIONS
        ...
        self.edit_cov_group = QLineEdit("COVADIS Bretagne")
        ...
        self.radio_cov_from_import = QCheckBox(
            "Utiliser les points importés (onglet Import) — colonne 'Code' comme chaîne COVADIS"
        )
        ...
        self.tbl_cov_codes.setHorizontalHeaderLabels(
            ["Code", "Famille / Libellé", "Calque COVADIS", "Géométrie"]
        )
        ...
        btn_run.clicked.connect(self._calc_covadis)
            ...
            for cd in bp.code_defs.values():
                if cd.code.endswith('#') and cd.libelle not in seen:
                    ...
                    for col, val in enumerate([
                        ...
                        item = QTableWidgetItem(val)
                        # Colorier selon la couleur COVADIS
    ...
    def _calc_covadis(self):
        """Décode les points et crée les couches QGIS."""
        if not HAS_COVADIS:
            QMessageBox.critical(self, "COVADIS", "Module COVADIS non disponible.")
            return
        ...
        if not pts_avec_code:
            QMessageBox.warning(
                self, "Pas de points",
                "Aucun point avec code COVADIS trouvé.\n\n"
                "Importez d'abord vos points via l'onglet Import.\n"
                "La colonne 'Code' doit contenir les chaînes de codes COVADIS "
        ...
        try:
            ...
            group_name = self.edit_cov_group.text().strip() or 'COVADIS Bretagne'

            self.txt_cov_log.clear()
            self.txt_cov_log.append(
                f"⏳ Traitement COVADIS '{key}' ({mode}) sur "
            ...
            self._log("═" * 55)
            self._log(f"  COVADIS Bretagne — {len(pts_avec_code)} points traités")
            stats = result['stats']
            ...
            QMessageBox.information(
                self, "COVADIS — Traitement terminé",
                f"✅ Traitement réussi !\n\n"
