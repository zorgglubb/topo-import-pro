# -*- coding: utf-8 -*-
"""
covadis/__init__.py — Géocodification COVADIS "Bretagne" (INRAP BZH 2017)
Auteurs : Mehdi Belarbi & Claude (Anthropic)

Ce module gère :
  1. CovadisParser      : lecture des fichiers .cod et .bpt
  2. CovadisDecoder     : décodage des chaînes de codes COVADIS sur les points
  3. CovadisGeomBuilder : reconstruction des géométries (polylignes, cercles,
                          rectangles 2pts) depuis les points décodés
  4. CovadisLayerFactory: création des couches QGIS avec attributs complets
  5. Registre 'bretagne' prêt à l'emploi

Logique de codification COVADIS Bretagne (INRAP BZH) :
...
class DecodedCode:
    ...
    definition  : Optional[CovadisCodeDef] = None


@dataclass
class PointCovadis:
    """Un point topographique avec sa chaîne COVADIS décodée."""
    pt_id       : str
    x           : float
    y           : float
    z           : Optional[float]
...
# ─────────────────────────────────────────────────────────────────────
class CovadisParser:
    """Lit un fichier .cod COVADIS et retourne un dict {code: CovadisCodeDef}."""

    def parse(self, filepath: str) -> Tuple[dict, Dict[str, CovadisCodeDef]]:
    ...
    def _parse_code(self, line: str) -> Optional[CovadisCodeDef]:
        p = line.split(';')
...
class CovadisDecoder:
    """
    Décode les chaînes de codes COVADIS sur chaque point.
    ...
    def __init__(self, code_defs: Dict[str, CovadisCodeDef]):
        self.code_defs = code_defs
        # Index par code_base pour recherche rapide
        self._base_idx: Dict[str, CovadisCodeDef] = {}
    ...
    def decode_point(self, pt_id: str, x: float, y: float, z: Optional[float],
                     chaine: str) -> PointCovadis:
        """Décode la chaîne de codes d'un point topographique."""
        pt = PointCovadis(pt_id=pt_id, x=x, y=y, z=z,
    ...
    def _find_def(self, base: str, full_code: str) -> Optional[CovadisCodeDef]:
        """Cherche la définition COVADIS correspondant à ce code."""
...
class CovadisGeomBuilder:
    ...
    def build(self, points: List[PointCovadis]) -> List[Structure]:
        """
        Construit toutes les structures depuis la liste ordonnée de points.
        """
        # Threads actifs : {(base, instance): [list of (PointCovadis, param)]}
    ...
    def _rect_2pts(self, p1, p2, largeur: float) -> List[Tuple]:
        ...
        Le rectangle est à gauche dans le sens de déplacement (convention COVADIS).
        """
        x1,y1 = p1[1],p1[2]
        x2,y2 = p2[1],p2[2]
        dx,dy = x2-x1, y2-y1
...
# ─────────────────────────────────────────────────────────────────────
class CovadisLayerFactory:
    """
    Crée les couches QGIS depuis les structures reconstruites.

    Stratégie :
      - Une couche par calque COVADIS (nom du calque = nom de la couche)
    ...
    def create_layers(self, structures: List[Structure],
                      ...
                      group_name: str = 'COVADIS Bretagne') -> List:
        """
...
class BretagneProcessor:
    """
    Point d'entrée unique pour la géocodification COVADIS Bretagne.
    ...
    def __init__(self, mode: str = '2D'):
        ...
        if not cod_path or not os.path.exists(cod_path):
            raise FileNotFoundError(
                ...
                f"Assurez-vous que les fichiers COVADIS sont dans : {_DIR}"
            )
        parser = CovadisParser()
        self.general, self.code_defs = parser.parse(cod_path)
        self.decoder = CovadisDecoder(self.code_defs)
        self.builder = CovadisGeomBuilder(
            self.code_defs,
            sep_codes  = self.general.get('SeparateurCodes', '-'),
            sep_param  = self.general.get('SeparateurParam', '/'),
        )
        self.factory = CovadisLayerFactory()

    def process(self, topo_points, crs: str = 'EPSG:2154',
                group_name: str = 'COVADIS Bretagne') -> dict:
        """
    ...
    @property
    def familles(self) -> Dict[str, List[CovadisCodeDef]]:
