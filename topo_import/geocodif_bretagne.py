# -*- coding: utf-8 -*-
"""
geocodif_bretagne.py — Codification GEOCODIF Bretagne (Inrap BZH 2017)
Auteurs : Mehdi Belarbi & Claude (Anthropic)

Ce module intègre la codification archéologique GEOCODIF utilisée
par l'Inrap Bretagne (ArchéoCOD 2017), et permet :

  1. De lire / décoder les codes GEOCODIF dans les fichiers topo importés
  2. D'enrichir les attributs des points (calque, couleur, libellé)
  3. De créer des couches QGIS stylisées par catégorie GEOCODIF
  4. D'exporter la codification vers les formats .cod et .bpt GEOCODIF
...
Structure des codes (séparateur ';') :
  ...
  [...] calque      : nom du calque AutoCAD/GEOCODIF
"""
...
# ═════════════════════════════════════════════════════════════════════
# STRUCTURE D'UN CODE GEOCODIF
# ═════════════════════════════════════════════════════════════════════
@dataclass
class GeocodifCode:
    """Un code de la codification GEOCODIF Bretagne."""
...
# ═════════════════════════════════════════════════════════════════════
@dataclass
class GeocodifPointLib:
    """Définition d'une bibliothèque de points GEOCODIF (.bpt)."""
    nom          : str
    version      : str
    calque_point : str
    couleur_point: int
...
# ═════════════════════════════════════════════════════════════════════
# CODIFICATION GEOCODIF BRETAGNE — DONNÉES INTÉGRÉES
# ═════════════════════════════════════════════════════════════════════
...
# ═════════════════════════════════════════════════════════════════════
def _build_registry() -> Dict[str, GeocodifCode]:
    """Construit le dictionnaire code → GeocodifCode."""
    registry = {}
...
GEOCODIF_REGISTRY: Dict[str, GeocodifCode] = _build_registry()

# ── Catégories triées ─────────────────────────────────────────────────
GEOCODIF_CATEGORIES: List[str] = sorted(set(
    c.categorie for c in GEOCODIF_REGISTRY.values()
))

# ── Codes par catégorie ───────────────────────────────────────────────
def get_codes_by_category(cat: str) -> List[GeocodifCode]:
    seen = set()
    result = []
    for c in GEOCODIF_REGISTRY.values():
...
# ═════════════════════════════════════════════════════════════════════
# DÉCODEUR DE CODE GEOCODIF
# ═════════════════════════════════════════════════════════════════════
class GeocodifDecoder:
    """
    Décode un code GEOCODIF depuis un champ texte d'un point topo.
    ...
    def decode(self, raw_code: str) -> Optional[GeocodifCode]:
        """
        Tente de résoudre un code brut vers un GeocodifCode.
        ...
        for key in [code_lower, code_lower + '#', code_lower.rstrip('0123456789')+'#']:
            if key in GEOCODIF_REGISTRY:
                return GEOCODIF_REGISTRY[key]
    ...
    def decode_point(self, point) -> Optional[GeocodifCode]:
    ...
    def enrich_points(self, points: list) -> list:
        """
        Enrichit une liste de TopoPoint avec les infos GEOCODIF.
        Ajoute les attributs geocodif_* si un code est reconnu.
        """
        for p in points:
            cov = self.decode_point(p)
            if cov:
                p.geocodif_libelle  = cov.libelle
                p.geocodif_calque   = cov.calque
                p.geocodif_categorie = cov.categorie
                p.geocodif_couleur  = cov.couleur_hex
                p.geocodif_type_geom = cov.type_geom_label
            else:
                p.geocodif_libelle  = ""
                p.geocodif_calque   = ""
                p.geocodif_categorie = ""
                p.geocodif_couleur  = "#808080"
                p.geocodif_type_geom = ""
        return points
...
# ═════════════════════════════════════════════════════════════════════
class GeocodifExporter:
    """Génère les fichiers .cod et .bpt compatibles GEOCODIF."""

    def generate_cod_2d(self) -> str:
        """Génère le contenu d'un fichier .cod 2D (texte latin-1)."""
        g = GEOCODIF_BRETAGNE_GENERAL
        lines = [
            "[General]",
            "Génération_table=5",
...
# ═════════════════════════════════════════════════════════════════════
# CRÉATEUR DE COUCHES QGIS PAR CATÉGORIE GEOCODIF
# ═════════════════════════════════════════════════════════════════════
class GeocodifLayerBuilder:
    """
    Crée des couches QGIS mémoire stylisées selon la codification
    GEOCODIF Bretagne, une couche par catégorie.
    """

    def build_layers(self, points: list, crs_code: str = "EPSG:2154"):
        """
        Crée et retourne une liste de QgsVectorLayer, une par catégorie GEOCODIF.
        Les points sans code GEOCODIF reconnu sont dans la couche "Non codifié".

        Parameters
        ----------
        points   : liste de TopoPoint (enrichis via GeocodifDecoder)
        ...
        for p in points:
            cat = getattr(p, 'geocodif_categorie', '') or 'Non codifié'
        ...
        for cat, pts in sorted(buckets.items()):
            ...
            layer = QgsVectorLayer(f"Point?crs={crs_code}", f"GEOCODIF — {cat}", "memory")
            ...
            for p in pts_valides:
                ...
                f.setAttributes([
                    ...
                    getattr(p, 'geocodif_categorie', cat),
                    getattr(p, 'geocodif_type_geom', ''),
                    getattr(p, 'geocodif_couleur',   '#808080'),
                ])
                feats.append(f)
        ...
        try:
            ...
            for p in points:
                ...
                if code in seen_codes:
                    continue
                seen_codes.add(code)
                couleur = getattr(p, 'geocodif_couleur', '#808080')
                libelle = getattr(p, 'geocodif_libelle', code)
...
# ═════════════════════════════════════════════════════════════════════
_decoder  = GeocodifDecoder()
_exporter = GeocodifExporter()
_builder  = GeocodifLayerBuilder()

def decode_code(raw: str) -> Optional[GeocodifCode]:
    """Décode un code GEOCODIF brut."""
    return _decoder.decode(raw)

def enrich_points(points: list) -> list:
    """Enrichit une liste de TopoPoint avec les attributs GEOCODIF."""
    return _decoder.enrich_points(points)

def get_all_codes() -> List[GeocodifCode]:
    """Retourne tous les codes GEOCODIF uniques."""
    seen = set()
    result = []
    for c in GEOCODIF_REGISTRY.values():
...
def build_qgis_layers(points: list, crs_code: str = "EPSG:2154"):
    """Crée les couches QGIS par catégorie GEOCODIF."""
